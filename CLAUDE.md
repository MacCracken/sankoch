...
- **Toolchain**: Cyrius 5.5.22 (`cyrius.cyml: cyrius = "5.5.22"`)
...
- **Cyrius stdlib** — `syscalls`, `string`, `alloc`, `fmt`, `vec`,
  `fnptr`, `thread`, `assert` (ships with Cyrius >= 5.5.22)
...
- **Toolchain pin**: `cyrius = "5.5.22"` in `cyrius.cyml`. CI and
...
- **No lock gate**: sankoch is stdlib-only (zero git deps), so there is
  ...
  toolchain version itself (`cyrius = "5.5.22"` in `cyrius.cyml`)
