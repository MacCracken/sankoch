...
- **CI rebuilt**: separate `Build & Test`, `Security Scan`, and
  ...
  `cyrius vet`, `cyrius deps --verify` (SHA hash gate against
  `cyrius.lock`), and a distlib-in-sync gate.
...
- `cyrius.lock` — SHA256 lockfile generated by `cyrius deps`. Pins
  every stdlib module for reproducible builds.
