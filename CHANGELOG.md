...
The profile bundles link again, and Brotli decode is added for rekha's WOFF2. Next up is
**2.8.1, a Brotli encoder**.

### Fixed

- **Every alloc-bearing profile bundle was unlinkable from 2.7.10 through 2.7.15.**
  - `runtime.cyr`'s arena guard called `_sankoch_reset_tables`, which only `lib.cyr` defined.
  - Any program that called into `dist/sankoch-{zlib,gzip,xz,bzip2,zstd,tar,zip,zipall}.cyr` was
    refused: "refusing to emit binary with 1 reachable undefined function(s)".
  - The fix:
    - each memoizing module owns `_<module>_reset_tables`;
    - `lib.cyr` and a new `src/reset_<profile>.cyr` per profile define the bundle's dispatcher over
      exactly the modules it carries.
  - Rule: [`docs/architecture/003-per-profile-reset-dispatch.md`](docs/architecture/003-per-profile-reset-dispatch.md).
  - Filing (rekha), archived:
    [`issues/archived/2026-09-15-profile-bundles-call-sankoch-reset-tables-outside-their-closure.md`](docs/development/issues/archived/2026-09-15-profile-bundles-call-sankoch-reset-tables-outside-their-closure.md).
- **Once the profiles linked, the reset turned out to be incomplete even in the full bundle.** The
  filing did not see this. Measured on 2.7.15 sources:
  - **Memoized globals the 2.7.10 reset missed.** After `alloc_reset()`, caller memory was overwritten
    by xz BT4 (**229,370 words**), zstd L9 (**712**) and zstd L6 (**14**). All are now zeroed.
  - **Unlocked public builders had no guard**, and consumers call them directly:
    - `crc64_init_table` / `crc64_init` (2,043 caller bytes rewritten);
    - `crc32_bzip2_init_table` (2,042);
    - `lz77_init` (524,288);
    - `huff_build_*` (8,701), while `huff_build_fixed` kept tables that sat inside a caller's buffer.

    Each now calls `_sankoch_arena_guard`.
  - **Stranded canary.** A caller that filled the first 256 MB arena chunk before sankoch's first call
    got a canary that `alloc_reset()` never zeroes, and 475,882 caller bytes were overwritten or
    `mutex_lock` hung. `_sankoch_canary_stranded` closes this exactly.
  - **AGNOS.** The guard ran after `_sankoch_lock`'s AGNOS early return, so on AGNOS the reset was
    dead code. It now runs first on every target.
  - **A failed 8-byte canary alloc disarmed detection** (review). A later `alloc_reset()` went unseen
    and `crc32` came out wrong. The canary now bypasses the fault-injection seam, and a genuine
    failure makes the next guard treat the unarmed epoch as reset.

### Added

- **Brotli decoder (RFC 7932, decode only)**:
  - `brotli_decompress(src, src_len, dst, dst_cap)`;
  - `brotli_decompress_capped(…, max_output)`: clamped to `dst_cap`, may exceed 16 MiB, same argument
    rules as `zlib_decompress_capped`;
  - `FORMAT_BROTLI = 9` via `decompress()`. `compress(FORMAT_BROTLI)` is `ERR_UNSUPPORTED_FORMAT`.

  Coverage and behaviour:
  - All of RFC 7932 with errata 6977: context modes, block switching, WBITS 10..24, the static
    dictionary and its 121 transforms.
  - Output bounds are checked at every meta-block header, so a bomb fails in O(1) and never returns
    a truncated success. The tighter of `dst_cap` and the ceiling reports first, as with zlib.
  - Bytes after the final padding are `ERR_CORRUPT_DATA`, as `brotli -d` does: Brotli has no checksum
    and no magic, so exact consumption is the integrity signal.
  - Large Window Brotli is `ERR_UNSUPPORTED_FORMAT`.

  Decisions: [`docs/adr/0001-brotli-decoder-placement.md`](docs/adr/0001-brotli-decoder-placement.md).
  Filing (rekha), archived as shipped:
  [`proposals/archived/2026-09-15-brotli-decoder-for-woff2.md`](docs/development/proposals/archived/2026-09-15-brotli-decoder-for-woff2.md).
  - **Profiles.**
    - Brotli is in the full `[lib]`.
    - `[lib.brotli]` → `dist/sankoch-brotli.cyr`.
    - `[lib.woff]` → `dist/sankoch-woff.cyr`: the `[lib.zlib]` closure + Brotli, so rekha gets
      WOFF 1.0 and WOFF2 from one bundle. One sankoch bundle per program: two profile bundles collide
      on `runtime.cyr`'s globals, and zlib+zstd segfaults after a reset.
    - 12 bundles total.
  - **Dictionary.** `src/brotli_dict.cyr` is generated by `scripts/brotli_dict2cyr.py` from
    `docs/sources/brotli/dictionary.bin`:
    - 122,784 B, sha256 `20e42eb1…5c70`, CRC-32 **`0x5136cb04`** (RFC 7932 Appendix A);
    - byte-identical to google/brotli v1.2.0;
    - CI regenerates it and `cmp`s; the decoder FNV-1a-checks it once per process.

    Constant tables are pinned by CRC against `docs/sources/brotli/tables.txt`. The extractor is
    re-runnable: `--src`, with input sha256 pins in the README.
  - **Validation.**
    - A 22,042-stream reference differential against `brotli -d` and a strict Python decoder:
      0 mismatches, same error classes.
    - `scripts/brotli-smoke.sh`: **1008/1008** (217 corpus rows, google testdata 45/45, a q0–11 x
      w10–24 sweep, the LiberationSans `-q 11 -w 22` stream of 169,278 B decoded exactly, and
      aarch64 under qemu).
    - `tests/tcyr/brotli_decompress.tcyr`: **4,554** assertions. Exact-cap decodes run with src
      flush against a PROT_NONE page and guard bytes after dst.
    - `fuzz/fuzz_brotli.fcyr`: 5,013 inputs, with guard-page src/dst.
  - **Throughput** (`cyrius bench`, Ryzen 7 5800H): alice29 q11 **113.8 MB/s**, LiberationSans q11 w22
    **83.1**, records **167.8**. See
    [`docs/benchmarks/2026-09-16-2.8.0-brotli.md`](docs/benchmarks/2026-09-16-2.8.0-brotli.md).
    SIZE rows unchanged.
- **Profile link gate.** `scripts/profile-link-gate.sh` runs in CI and release, on every profile
  parsed from `cyrius.cyml`. For each it checks:
  - reset registration;
  - a reachable-call link probe;
  - reference-CLI vectors decoded before and after `alloc_reset()`, with a victim buffer holding the
    old arena;
  - a lock-only `--agnos` DCE probe that must keep the reset alive.

  A profile with no probe fails the gate.
- **Gates**:
  - `scripts/nul-literal-gate.py` (CI): no string literal may decode to NUL, except the generated
    dictionary ([`docs/architecture/004-string-literal-nul-rule.md`](docs/architecture/004-string-literal-nul-rule.md));
  - a CI dictionary regen `cmp`;
  - `git ls-files` tracking checks for every bundle.
- `arena_reset.tcyr` cells for the builder guards, the stranded canary (both directions) and the
  failed canary arm.

### Changed

- **Toolchain `6.6.2` → `6.6.4`, `lib/` re-vendored.**
  - The stdlib syscall peers gained `SYS_FLOCK` and `O_DIRECT` / `O_LARGEFILE` / `O_DIRECTORY` /
    `O_NOFOLLOW`.
  - `cyrius.lock` gains a pin trailer. It is still gitignored here: stdlib-only, no lock gate.
- CI's dist gate and release's regenerate and archive steps take the profile list from
  `profile-link-gate.sh --list-profiles`, not from hand-kept YAML.
- `tests/tcyr` now 27 suites, **4,500,520 passed / 0 failed**. Source **16,471 → 18,503** lines
  (23 domain modules + 10 reset dispatchers); full bundle 16,518 → 18,375 lines.
- **Upstream cyrius issues filed, with sankoch stopgaps:**
  - string-literal interning aliases NUL-bearing literals → the NUL-literal rule + gate;
  - DCE keeps dead literal and global data (a `zlib_compress`-only full-bundle consumer with
    `CYRIUS_DCE=1` grows **74,352 → 213,072 B**) → the lean profiles do not carry Brotli;
  - the `#define` table is capped at 16 → per-profile reset files instead of module markers.

### Security

- The reset fixes above are memory-safety fixes. Before 2.8.0, calling `alloc_reset()` under a
  consumer of xz, zstd, the unlocked builders, or a stranded canary let sankoch write through
  caller-owned memory.
- **Pre-release review**
  ([`docs/audit/2026-09-16-2.8.0-brotli-and-reset.md`](docs/audit/2026-09-16-2.8.0-brotli-and-reset.md)):
  0 HIGH · 4 MEDIUM · 15 LOW · 4 INFO, all fixed except one upstream-doc note (R-22). The mediums
  were:
  - `fuzz_brotli` and the tcyr could not detect out-of-bounds reads or writes: no guard-placed
    buffers, and two mutants passed. Both now crash the harnesses.
  - The link gate's AGNOS check missed the 2.7.10–2.7.15 regression in 9 of 11 profiles. The
    lock-only probe now fails 11/11, and every alloc-bearing 2.7.15 bundle fails.
  - T12's wall-clock IMTF budget failed under load and qemu. It is now a thread-CPU ratio against a
    bytewise reference.

  The libbrotlidec 200k mutation differential was not run for this release.
...
[`2026-09-07-streaming-zlib-decoder-hangs-on-dynamic-huffman-codes.md`](docs/development/issues/archived/2026-09-07-streaming-zlib-decoder-hangs-on-dynamic-huffman-codes.md)
