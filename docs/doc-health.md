...
> **Last refresh**: 2026-06-16 (v2.4.1 cut — xz/LZMA optimal-parse encode shipped) | **Refresh cadence**: opportunistic — update the affected row whenever a doc is touched.
>
...
| `CHANGELOG.md` | 2026-06-16 | ✅ Fresh | **Source of truth per CLAUDE.md.** Through `[2.4.1] — 2026-06-16` (xz/LZMA optimal-parse encode — `xz_compress` + `compress(FORMAT_XZ)`). Empty `[Unreleased]` header at top per Keep-a-Changelog convention. |
...
| `VERSION` | 2026-06-16 | ✅ Fresh | `2.4.1`. Single source of truth per the standards. |
...
| File | Last touched | Status | Notes |
|---|---|---|---|
| `roadmap.md` | 2026-06-16 | ✅ Fresh | **2.4.1 shipped — xz/LZMA optimal-parse encode.** Ladder: 2.4.0/2.4.1 ✅, 2.4.2 bzip2 decode next, 2.4.3 bzip2 encode. File Summary refreshed (`src/xz.cyr` 1,738; total 8,562). Test/fuzz/distlib figures updated. |
| `state.md` | 2026-06-16 | ✅ Fresh | Refreshed for 2.4.1. Version 2.4.1, xz codec complete (in-flight = 2.4.2). Source 8,562 / 15 modules; tests 200 fns / 4,326,194 asserts; fuzz 2,449 / 3 files; dist 8,541 + 314. SIZE gate note: xz encode excluded. |
