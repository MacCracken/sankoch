...
| `VERSION` | 2026-07-21 | ✅ Fresh | `2.7.5`. Single source of truth per the standards. |
| `cyrius.cyml` | 2026-07-20 | ✅ Fresh | Toolchain pin `cyrius = "6.4.69"` (6.4.67 → 6.4.68 at 2.7.0, → 6.4.69 at 2.7.1, tracking the active toolchain; 2.7.1 built + tested on it. `cyrius deps` re-resolved, clean rebuild + all gates clean — no source/API/wire-format change from the pin). |
