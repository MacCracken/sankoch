...
> **Last refresh**: 2026-08-23 (**2.7.9 cut** — DEFLATE sync flush for RFC 7692 (`deflate_enc_flush` /
`deflate_enc_reset_context` / `deflate_dec_produced`, `deflate.cyr` 2,596 → 2,855) plus the
fixed-vs-dynamic block chooser measuring it forced (RFC 7692 framing +64 % → +0.1 % vs reference zlib;
3 of 43 batch SIZE rows improved, 40 unchanged) and a latent M-8-class OOM-latch fix in
`_deflate_build_enc_fixed`. New `docs/benchmarks/2026-08-23-2.7.9-sync-flush.md`; new
`tests/tcyr/deflate_sync_flush.tcyr` (24th file) + `scripts/deflate-flush-smoke.sh` +
`programs/deflate_flush_smoke.cyr` (reference interop vs Python `zlib`, both directions); the bote
issue resolved + archived (open queue 0). Toolchain pin 6.5.26 → 6.5.35 — ⚠ **not** tooling-neutral:
bare `cyrius fmt <file>` now rewrites in place instead of printing to stdout, and `--check` finally
reports drift by exit code, so the 2.2.4–2.7.8 CI format gate diffed empty-vs-file *while silently
mutating the checkout*; moved to `--check` and the tree reformatted (24 files, continuation-indent
only). `.gitignore` `lib/*.cyr` + `!lib/k*.cyr` → `/lib/` (the glob missed subdirectories, so
`lib/unicode/` had been committed by accident; neither it nor `keccak.cyr` is referenced or resolved
any more). **This sweep also closed a real ledger gap**: state.md's Recent-releases table stopped at
2.7.5 and roadmap still carried the long-shipped 2.7.6 as **▶ Next** — 2.7.6/2.7.7/2.7.8 rows added,
roadmap's Next section replaced with a forward 2.8.0 entry. Figures re-counted (source 16,265; full
bundle 16,312; fuzz 7,589; tests 317 fns / 4,495,197). Previously — **2.7.5 cut + 2.8.x-prep doc sweep** — 2.7.5 shipped the zstd L9 hash-chain saturation cutoff (`_zo_getmatches`, `zstd.cyr` 3,028 → 3,058; record L9 2.76 → 1.86 s, ratio Δ ≤ +0.043 %; new architecture note [`001-zstd-optimal-chain-cutoff.md`](architecture/001-zstd-optimal-chain-cutoff.md) — the **first filed architecture note**). This sweep re-counted stale figures across state.md + roadmap (full bundle 14,843 → 15,792; dist xz 2,799 → 3,074, zstd 2,514 → 3,188, tar 11,363 → 12,312, zipall 11,359 → 12,308; fuzz 7,499 → 7,529, `fuzz_zstd` 1,680), refreshed this ledger's badly-lagged state.md row (was stuck at 2.5.8) + added the architecture-note-001 row + updated VERSION/README/CHANGELOG/roadmap rows, added the 2.7.4 cross-block milestone to README's zstd paragraph, and **opened the roadmap ▶ 2.8.x ladder** (2.8.0 SIMD CRC-32 `PCLMULQDQ`, 2.8.1 GPU texture compression — taken down from the Backlog, which keeps DEFLATE match-finder + Brotli; opens straight into the feature, no P(-1) lead — a **P(-1) hardening pass closes out the 2.8.x line** instead, before the next minor). Previously — **2.7.4 cut — zstd encoder cross-block match window**; both parses (greedy `_ze_lz_parse` + DP-optimal `_zo_lz_parse`) restructured to frame-global coordinates + a persistent 512 KiB window (`_ze_prev` 4 MiB) + `_ze_chain_snapshot/restore/fill`, in `zstd.cyr` (2,923 → 3,028, +105); a block's sequences reference matches in prior blocks; no frame/offset change (single-segment frames); record L9 227,721 → 125,886 (−44.7 %, **beats `zstd -19`**), source −10.5 %, single-block byte-identical, all round-trip via reference `zstd -d`; design + adversarial review ran as a workflow; an OOM-sweep crash (NULL `_ze_head_save`/`_ze_prev`) caught + guarded; new `docs/benchmarks/2026-07-21-2.7.4-zstd-window.md` (window sweep); `zstd-encode-smoke.sh` +1 cross-block fixture (16 cases), `fuzz_zstd.fcyr` +40 cross-block round-trips; **completes the encoder ladder**; VERSION/CHANGELOG/state/roadmap → 2.7.4 (pin unchanged 6.4.69); roadmap ▶ Scheduled section removed — no committed next release, forward list is just the Backlog; source 15,715; test totals 293 fns / 4,484,493 (unchanged); fuzz 7,539 across 6 files; full bundle 15,762. Previously — **2.7.3 cut — zstd encoder DP optimal parse (levels 7–9)**; DP optimal parser `_zo_lz_parse` + helpers in `zstd.cyr` (2,384 → 2,923, +539) gated behind `zstd_compress_level` 7–9; **per-block best-of** (`_ze_try_seq_block` + `_ze_tmp2`) makes it a strict improvement over the greedy default (real-source L9 −4.9 %, records best-of-neutral); the 2.5.8 DP probe integrated + hardened + reference-validated; new `docs/benchmarks/2026-07-21-2.7.3-zstd-optimal.md`; permanent coverage — `fuzz_zstd.fcyr` +500 optimal-parse round-trips (levels 7–9), `zstd-encode-smoke.sh` now reference-decodes every fixture at both L6 and L9 (`programs/zstd_encode_smoke.cyr` writes a second L9 frame); validated vs reference `zstd -d` (40 adversarial files + 800 round-trips, 0 fail); **completes the 2.7.x ladder**; VERSION/CHANGELOG/state/roadmap → 2.7.3 (pin unchanged 6.4.69); roadmap ladder emptied to conditional + the newly-surfaced zstd-window gap; source 15,610; test totals 293 fns / 4,484,493 (unchanged); fuzz 7,499 across 6 files; full bundle 15,657. Previously — **2.7.2 cut — xz encoder dictionary/window growth**; xz-private match window 32 KB → 256 KB (`XZE_WINDOW`) + `BT_SON_SLOTS` 2×window + `XZE_DICT_CODE` = 12 (256 KB LZMA2 dict), in `xz.cyr` (2,104 → 2,111); real-source corpus 58704 → 54976 B (−6.4 %), gap to `xz -6` +7.0 % → +0.2 %; son[] → 8 MB (lazy, encode-only); DEFLATE's 32 KB `LZ77_WINDOW` frozen (deflate/gzip/zlib suites green); reference `xz -d` accepts every stream (dict ≥ max distance); **completes the xz-encode arc**; 2.7.0-baseline benchmark doc gained a 2.7.2 results + window-sweep section; VERSION/CHANGELOG/state/roadmap → 2.7.2 (pin unchanged at 6.4.69); roadmap ladder now just 2.7.3 = zstd optimal parse; source 15,071; test totals 293 fns / 4,484,493 (unchanged — encoder change, no new tests); fuzz 6,999 across 6 files; full bundle 15,118. Previously — **2.7.1 cut — xz encoder BT4 match finder**; binary-tree finder (LzFind.c-style) on xz-private tables (65536-slot son[] + hash heads) with a seed-only O(1) skip, in `xz.cyr` (1,939 → 2,104); real-source corpus +21 % (0.60 → 0.73 MB/s) and *better* ratio (58872 → 58704 B), gap to `xz -6` 7.1× → 5.8×, repetitive neutral, DEFLATE lz77 byte-identical, reference `xz -d` preserved; an HC4 attempt was tried + rejected first (the "78 % match finder" figure was operation-count, not time); research + adversarial design (3 verifiers, all *sound*) ran as a workflow; 2.7.0-baseline benchmark doc gained a 2.7.1 results section; dropped `lz77_init()` from the xz encode path; toolchain pin 6.4.68 → 6.4.69; VERSION/CHANGELOG/state/roadmap → 2.7.1; roadmap ladder re-cut (2.7.2 = xz dictionary/window growth, 2.7.3 = zstd optimal parse); source 15,064; test totals 293 fns / 4,484,493 (unchanged — encoder change, no new tests); fuzz 6,999 across 6 files; full bundle 15,111. Previously — **2.7.0 cut — xz encoder repetitive-data speedup**; rep-only `nice_len` greedy shortcut + interior DP cut in `xz.cyr` (1,836 → 1,939); text/zeros encode ~290–473× faster and smaller, corpus neutral, reference `xz -d` round-trip preserved; research + adversarial design ran as a workflow; new `docs/benchmarks/2026-07-20-2.7.0-baseline.md` (before + after); toolchain pin 6.4.67 → 6.4.68; VERSION/CHANGELOG/state/roadmap → 2.7.0; roadmap ladder re-cut (2.7.1 = HC4 match finder for the real-source 7.3× gap, 2.7.2 = zstd optimal parse); source 14,899; test totals 293 fns / 4,484,493 (unchanged — encoder speed, no new tests); fuzz 6,999 across 6 files; full bundle 14,946. Previously — **2.6.4 cut — P(-1) hardening: first security audit of the ZIP surface**; new `docs/audit/2026-07-20-zip-container.md` [0 HIGH · 3 MEDIUM · 1 LOW confirmed, all fixed — i64 Zip64-bounds overflow ×4 → subtraction-form, streaming-abandon lock leak → `_zip_abandon`, mid-stream-add overlap + name-length truncation → rejected; HIGH symlink claim rebased LOW: ZIP is memory-only]; new `fuzz/fuzz_zip.fcyr` (6 strategies, raises fuzz harness count 5 → 6); `zip.tcyr` 175 → 206 assertions; `zip-smoke.sh` 13 checks unchanged; VERSION/CHANGELOG/state/roadmap → 2.6.4; source 14,796 [zip.cyr 1,206]; test totals 293 fns / 4,484,493; fuzz 6,999 across 6 files; full bundle 14,843. Previously — **2.6.3 cut — ZIP streaming write + per-entry metadata**; completes the 2.6.x ZIP arc; `zip.tcyr` 20 tests / 175 assertions; `zip-smoke.sh` 13 checks incl. metadata, data descriptors and a streaming-write check; VERSION/CHANGELOG/state/roadmap → 2.6.3; source 14,601; test totals 287 fns / 4,484,462; roadmap notes the un-audited ~1,900 ZIP lines for the next P(-1). Previously — **2.6.2 cut — ZIP Zip64**; EOCD record + locator + extended-information extra field, read + write; entry cap 65,535 → 16,777,216; latent 2.6.0 name-aliasing fix; `zip.tcyr` 16 tests / 123 assertions; `zip-smoke.sh` 10 checks incl. two Zip64 read fixtures + a Zip64 write check; VERSION/CHANGELOG/state/roadmap → 2.6.2; source 14,313; test totals 283 fns / 4,484,410. Previously — **2.6.1 cut — ZIP: every method**; new `src/zip_methods.cyr` (174) + `[lib.zipall]` profile (10 bundles); `zip.tcyr` 15 tests / 98 assertions; `zip-smoke.sh` 7 shapes incl. a bsdtar-written xz archive; VERSION/CHANGELOG/state/roadmap → 2.6.1; CLAUDE.md gained the zip_methods.cyr row + bsdtar in the reference-CLI rule; source 14,135 across 21 modules; test totals 282 fns / 4,484,385. Previously — **2.6.0 cut — ZIP archive container**; new `src/zip.cyr` (487) + `tests/tcyr/zip.tcyr` (22nd suite) + `scripts/zip-smoke.sh` + `[lib.zip]` profile (9 bundles); VERSION/CHANGELOG/state/roadmap → 2.6.0; CLAUDE.md architecture block gained the zip.cyr row + the ZIP reference-CLI rule; source 13,926 across 20 modules; test totals 278 fns / 4,484,346. Previously — **2.5.10 cut — P(-1) audit remainder**; closes the deferred half of the audit [zstd arena leaks → 0, tar ladder DoS, multi-frame zstd, multi-member gzip, L-2/L-3/L-4/R-1, L-5/I-1 testability]; **every confirmed audit finding resolved — 2.6.0 cleared**; VERSION/CHANGELOG/state/roadmap → 2.5.10; source 13,437 [zstd.cyr 2,384]; full bundle 13,480; test totals 267 fns / 4,484,286; fuzz 5,379 across 28 fns; INFO-D narrowed to the arena design item, INFO-I1 CLOSED. Previously — **2.5.9 cut — P(-1) security hardening**; first audit of the never-audited 2.4.x/2.5.x surface [xz/bzip2/tar/zstd-encoder] → new `docs/audit/2026-07-19-pre-2.6.0.md` [1 HIGH + 13 MEDIUM + 5 LOW] + `docs/benchmarks/2026-07-19-2.5.9-p1-baseline.md`; VERSION/CHANGELOG/state/roadmap → 2.5.9; source re-counted [tar.cyr 513→701, total 13,099; full bundle 13,142]; test totals 267 fns / 4,484,226; INFO-E resolved [negative], INFO-D amplified, INFO-I1 new; remainder → 2.5.10. Previously — **2.5.8 cut — zstd encoder priced parse**; VERSION / CHANGELOG / state / roadmap bumped to 2.5.8, source re-counted [zstd.cyr 1,992→2,058; total 12,845; full bundle 12,827], test totals 267 fns / 4,484,022 — **and corrected a long-standing under-count**: every historical assertion figure was 21 low, because the tally pattern dropped the one suite whose summary line omits the `(N total)` suffix, new benchmark doc `2026-07-19-2.5.8-priced-parse.md`, roadmap 2.5.8 shipped + the optimal/2-pass DP moved to a Deferred arc with the measured comparison that justified deferring it; roadmap File Summary re-counted against `wc -l` — it had drifted to a 12,248 total and stale Tests/Fuzz/Distlib paragraphs) | **Refresh cadence**: opportunistic — update the affected row whenever a doc is touched.
>
...
|---|---|---|---|
| `README.md` | 2026-08-23 | ✅ Fresh | **2026-08-23 (2.7.9)**: DEFLATE paragraph gained the RFC 7692 sync flush (`deflate_enc_flush` + the context-takeover pair) and the fixed-vs-dynamic block chooser. Prior — **2026-07-21 (2.7.5-cut) sweep**: zstd paragraph gained the **2.7.4 cross-block match window** (a block's sequences reference matches in prior blocks — sankoch's zstd now **beats `zstd -19` on record data**, L9 −44.7 %, single-block byte-identical). Prior 2026-07-21 sweep: zstd paragraph gained the 2.7.3 DP optimal parse (levels 7–9, best-of); xz paragraph gained the 2.7.1 BT4 finder + 2.7.2 256 KB window + the ~0.2 %-of-`xz -6` ratio + the sha256-rejection note; a new **Containers** section documents the tar cursor + PKZIP `.zip` (v2.6.x, absent before); the Architecture module table added the missing `zstd.cyr` / `tar.cyr` / `zip.cyr` / `zip_methods.cyr` / `runtime.cyr` rows and refreshed the `xz.cyr` row. Prior — Formats table + codec paragraphs updated at 2.5.5 to add **zstd** (de+compress, `FORMAT_ZSTD`) — was stale (missing zstd/tar since 2.5.0). Current through 2.4.6 otherwise: ratio-cap API subsection (batch `*_with_ratio_cap` + streaming `*_dec_init_capped`); Architecture table rows note ratio-cap on deflate/zlib/gzip + the agnos lock no-op on lib. Defers all volatile line / test / assertion / distlib counts to state.md (no inlined figures). |
| `CHANGELOG.md` | 2026-08-23 | ✅ Fresh | **Source of truth per CLAUDE.md.** Through `[2.7.9] — 2026-08-23` (DEFLATE sync flush for RFC 7692 — `deflate_enc_flush` / `deflate_enc_reset_context` / `deflate_dec_produced`, closing the bote filing; the fixed-vs-dynamic block chooser measuring it forced, +64 % → +0.1 % vs reference zlib; a latent M-8-class OOM latch in `_deflate_build_enc_fixed` found by the retry sweep; pin → 6.5.35; CI fmt gate → `--check`; `lib/keccak.cyr` + `lib/unicode/` removed); `[2.7.8] — 2026-08-18` (toolchain catch-up to 6.5.26, ten bundles byte-identical, issue queue cleared); `[2.7.7] — 2026-08-09` (ZIP sizing API + reclaimable readers + discriminated open failures, from the agnosai report); `[2.7.6] — 2026-07-26` (>1 MiB DEFLATE block-boundary corruption fix, filed by stiva); `[2.7.5] — 2026-07-21` (zstd L9 optimal-parse hash-chain saturation cutoff — `_zo_getmatches` bails after 128 consecutive non-improving candidates once `best >= 32`; record L9 2.76 → 1.86 s, ratio Δ ≤ +0.043 %, 43 gated SIZE lines untouched; cross-block L9 fuzz 3 → 10 calls); `[2.7.4] — 2026-07-21` (zstd encoder cross-block match window — both parses lifted to frame-global coordinates + a 512 KiB window; sankoch's zstd now beats `zstd -19` on record data (L9 −44.7 %), source −10.5 %, single-block byte-identical, no frame/offset change; completes the encoder ladder); `[2.7.3] — 2026-07-21` (zstd encoder DP optimal parse — levels 7–9, per-block best-of so never worse than the greedy default; real-source L9 −4.9 %; validated vs reference `zstd -d`; +500 fuzz + L9 smoke); `[2.7.2] — 2026-07-20` (xz encoder dictionary/window growth — xz-private match window 32 KB → 256 KB + 256 KB LZMA2 dict; real-source corpus −6.4 %, gap to `xz -6` +7 % → +0.2 %; completes the xz-encode arc); `[2.7.1] — 2026-07-20` (xz encoder BT4 binary-tree match finder — xz-private, seed-only skip; real-source +21 % and better ratio, repetitive neutral, DEFLATE lz77 untouched; HC4 tried + rejected; pin → 6.4.69); `[2.7.0] — 2026-07-20` (xz encoder repetitive-data speedup — rep-only `nice_len` greedy shortcut + interior DP cut, ~290–473× faster on text/zeros and smaller, corpus neutral; pin → 6.4.68); `[2.6.4] — 2026-07-20` (P(-1) hardening — first security audit of the ZIP surface: i64 Zip64-bounds overflow, streaming-abandon lock leak, mid-stream-add overlap, name-length truncation — all fixed; `fuzz_zip.fcyr`); `[2.6.3] — 2026-07-19` (ZIP streaming write + per-entry metadata — completes the ZIP arc); `[2.6.2]` ZIP Zip64 read + write + the latent name-aliasing fix); `[2.6.1]` ZIP: every method sankoch owns — 12/93/95 both ways, `[lib.zipall]`); `[2.6.0]` ZIP archive container — `zip.cyr` in-memory reader + writer, agnosai `.agpkg` core); `[2.5.10]` P(-1) audit remainder — zstd arena leaks → 0, tar ladder DoS, multi-frame zstd, multi-member gzip, testability; clears 2.6.0); `[2.5.9]` P(-1) security hardening (1 HIGH tar symlink-chain traversal + xz OOB/DoS/sha256 + OOM-latch class + zstd concurrency lock + stream allocs); `[2.5.8]` zstd encoder priced parse; `[2.5.7]` parse quality (repcode matching + adaptive FSE sequence tables); `[2.5.6]` competitiveness + decoder hardening; `[2.5.5]` sovereign zstd encoder; `[2.5.4]` xz/bzip2 throughput; `[2.5.3]` xz/bzip2 ratio cap; `[2.5.2]` pin → 6.4.66; `[2.5.1]` per-codec distlib profiles; `[2.5.0]` sovereign zstd + tar cursor. Empty `[Unreleased]` header at top per Keep-a-Changelog convention. **2.5.7 closeout restored the `[2.5.6]` header** (a bite's Edit had dropped it, orphaning 2.5.6 content under `[Unreleased]`). |
...
| `SECURITY.md` | 2026-08-23 | ✅ Fresh | **2026-08-23 (2.7.9)**: Supported Versions table was still at 2.5.x — four minors stale, so it named an unsupported line as supported. Now 2.7.x supported / 2.6.x security-fixes-only / < 2.6.0 upgrade, plus an explicit ⚠ pointing >1 MiB one-shot-compress users at 2.7.6 regardless of the table (pre-2.7.6 returned a longer-than-input stream with no error). Prior — Supported Versions table at 2.5.x. "Next periodic audit" pointer genericized to the pre-next-minor P(-1) closeout (was a stale/dangling "2.4.x decode-only xz/bzip2 arc" ref — xz/bzip2 encode had since shipped @2.4.1/2.4.3 and roadmap no longer listed that item). |
...
| `VERSION` | 2026-08-23 | ✅ Fresh | `2.7.9`. Single source of truth per the standards. |
| `cyrius.cyml` | 2026-08-23 | ✅ Fresh | Toolchain pin `cyrius = "6.5.35"` (6.4.69 at 2.7.1 → 6.5.16 → 6.5.26 at 2.7.8 → 6.5.35 at 2.7.9, tracking the active toolchain; 2.7.9 built + tested on it. `cyrius deps` re-resolved, clean rebuild + all gates clean — no source/API/wire-format change from the pin itself, but see the ⚠ `cyrius fmt` behaviour change in the header note. |
| `.gitignore` | 2026-08-23 | ✅ Fresh | Matches the first-party standard: `/dist/` (with the ten committed bundles re-included), `*.tar.gz`, `cyrius-*.tar.gz`, `SHA256SUMS`, `.env*` / `*.pem` / `*.key`. **2026-08-23 (2.7.9)**: `lib/*.cyr` + `!lib/k*.cyr` → `/lib/`. The glob never matched subdirectories, so `lib/unicode/` was committed by accident at `c93cefe`, and the un-ignore tracked a `keccak.cyr` nothing references; 6.5.35's dep resolution materialises neither. `cyrius deps` owns `lib/`, so nothing under it belongs in the tree. |
...
| File | Last touched | Status | Notes |
|---|---|---|---|
| `roadmap.md` | 2026-08-23 | ✅ Fresh | **2026-08-23 (2.7.9-cut)**: the **▶ Next** section still carried 2.7.6 — shipped a month earlier — in full detail, which the forward-only convention forbids; replaced with a forward **▶ 2.8.0** entry noting that nothing preempts the ladder, the issue queue is 0, and 2.7.9's chooser/pricer is unaudited surface for the 2.8.x-closeout P(-1) pass (with two specific things to look at: `_dyn_header_bits`'s fixed 512-byte buffer resting on an argued 281-byte bound, and the M-8 latch class the chooser exposed). Status header → v2.7.9; File Summary re-counted (deflate.cyr 2,545 → 2,855, lib.cyr 265 → 273, source 16,265, full bundle 16,312, fuzz 7,589; "as of 2.7.5" → 2.7.9; pin → 6.5.35). Prior — **2026-07-21 (2.7.5-cut) 2.8.x prep**: the **▶ Scheduled** section re-opened for the 2.8.x ladder — **2.8.0 SIMD CRC-32 (`PCLMULQDQ`)** (3-step approach: x86_64 fold / aarch64 PMULL-or-slice-by-8-fallback / byte-for-byte wire-identical proof + differential fuzz) and **2.8.1 GPU texture compression** (home-decision-first: sankoch vs mabda, since it is lossy), then a **P(-1) hardening pass at the end of the 2.8.x line** (audits the un-audited 2.7.x encoder surface + the 2.8.x additions before the next minor) — the two features taken down from the Backlog, which now keeps only DEFLATE match-finder + Brotli; Status header + intro + footer rewritten; File Summary re-counted at the 2.7.5 cut (fuzz 6,999 → 7,529 / `fuzz_zstd` 1,680; full bundle 14,843 → 15,792; dist xz 2,799 → 3,074 / zstd 2,514 → 3,188 / tar → 12,312 / zipall → 12,308; "as of 2.5.8" header → 2.7.5). Prior — **2026-07-21 post-2.7.3 re-org (forward-only)**: the shipped 2.7.0–2.7.3 encoder ladder was removed; **2.7.4 = zstd window growth** promoted from newly-surfaced to the one committed **▶ Scheduled** item (with a 3-step approach mirroring 2.7.2 + the `windowLog`-first decodability invariant); the parked work (SIMD CRC-32, DEFLATE match-finder, Brotli, GPU texture) collected into a single **Backlog — unscheduled (to be re-organised)** section; the Future-codecs section became "New-codec context" (modular-by-profile framing + the primitive-sources table the backlog references); intro + footer rewritten; File Summary re-counted at the 2.7.3 cut (zstd.cyr 2,384 → 2,923, total → 15,610). Prior — Status header (→ Stable v2.5.2) + Dependencies pin at 6.4.66. **File Summary re-counted** for the 19-module tree — added `runtime.cyr` (73) / `zstd.cyr` (729) / `tar.cyr` (513) rows, corrected `lib.cyr` 282→239 + `types.cyr` 41→42 + core-total 316→317, total 10,078→11,351; Distlib paragraph refreshed to the current eight bundles (full 11,394 / core 331 + six lean profiles). Anchor `#file-summary-at-230` kept stable. **Ladder scheduled 2026-07-18**: intro "no release committed" → committed ladder; Backlog section became `▶ Scheduled` with 2.5.3 (xz/bzip2 ratio cap) / 2.5.4 (encoder throughput) / **2.5.5 (zstd encode)** / 2.6.x (full-feature ZIP archive container arc, agnosai-first: 2.6.0 store+DEFLATE round-trip scoped from agnosai's `definitions/packaging.rs`, then 2.6.1 other methods [incl. zstd method 93, writable once 2.5.5 lands] / 2.6.2 zip64 / 2.6.3 streaming+metadata; encryption/multi-disk/Deflate64 non-goals). **Future reframed**: Zstandard removed (now scheduled), section retitled "additional codecs (in-scope, unscheduled)" per the modular-by-profile principle — Brotli + GPU-texture remain; Deferred untouched. **2.5.4 shipped 2026-07-18**: status → v2.5.4; 2.5.3 + 2.5.4 moved from the ladder to ✅-shipped notes (ladder now 2.5.5 → 2.6.x); File Summary re-counted (xz 1,819 / bzip2 1,316 / total 11,509). **2.5.6 shipped 2026-07-18**: status → v2.5.6; 2.5.5 + 2.5.6 moved to ✅-shipped; ladder now **2.5.7 (zstd repcode-aware matching)** → 2.6.x; intro updated (competitiveness landed). **2.5.7 shipped 2026-07-18**: status → v2.5.7; 2.5.7 → ✅-shipped (repcode + adaptive FSE tables); ladder now **2.5.8 (optional optimal parse)** → 2.6.x; intro notes the encoder beats `zstd -3`. **2.5.8 shipped 2026-07-19**: status → v2.5.8; 2.5.8 → ✅-shipped (priced parse); the optimal/2-pass parse became a **`⏸ Deferred`** section carrying the measured DP-vs-shipped comparison; ladder now 2.6.x only. **File Summary re-counted against `wc -l`** (it had drifted badly — total 12,248 → 12,845, zstd.cyr 1,462 → 2,058, lib.cyr 239 → 246) and the stale Tests / Fuzz / Distlib paragraphs beneath it refreshed (234 fns → 267; 4 fuzz files → 5; distlib full 11,394 → 12,827). |
| `state.md` | 2026-08-23 | ✅ Fresh | **Current at v2.7.9** (source 16,265 / 21 modules, dist full 16,312 + ten profiles, test 317 fns / 4,495,197, fuzz 7,589). **2026-08-23 (2.7.9-cut) sweep**: version/pin/tag/date, Source (deflate.cyr 2,596 → 2,855, lib.cyr 267 → 273), Test totals (22 split suites, +`deflate_sync_flush.tcyr`), Fuzz totals (7,529 → 7,589, `fuzz_deflate` 1,629 → 1,689), all ten Dist-bundle line counts, In-flight slots, and the Consumers table (a `bote` row — sankoch's half shipped, still blocked upstream on a cyrius `ws_server` handshake hook). **Closed a real gap**: the Recent-releases table stopped at 2.7.5, so 2.7.6 / 2.7.7 / 2.7.8 rows were added alongside 2.7.9. Prior — **Current at v2.7.5** (source 15,745 / 21 modules, dist full 15,792 + ten profiles, test 293 fns / 4,484,493, fuzz 7,529). **2026-07-21 (2.7.5-cut) sweep**: corrected stale Dist-bundle figures (full 14,843 → 15,792, xz 2,799 → 3,074, zstd 2,514 → 3,188, tar 11,363 → 12,312, zipall 11,359 → 12,308), fuzz total 7,499 → 7,529 (`fuzz_zstd` → 1,680), Released date 2026-07-20 → 2026-07-21, and rewrote **In-flight slots** from "no committed next release" to the **▶ 2.8.x ladder** (2.8.0 SIMD CRC-32, 2.8.1 GPU texture). This ledger row had lagged (older notes below stop at 2.5.2/2.5.8 — retained for history). Prior — Version/pin/date fields at 2.5.2 (pin 6.4.66). **Source + Dist bundles rows re-counted** — Source 10,078/16-module → 11,351/19-module; Dist table refreshed (full 11,394 / core 331 / zlib 4,924) + five per-codec profile rows added (gzip 5,077 / xz 2,697 / bzip2 2,014 / zstd 782 / tar 9,308), regeneration note corrected to all eight profiles. Clears the pre-2.4.7 stale-figure debt flagged since 2.5.0. Test/fuzz totals unchanged (234 fns / 4,483,834 asserts; 3,929 fuzz). **Ladder pass 2026-07-18**: In-flight slots rewritten from "none committed" to the scheduled 2.5.3 / 2.5.4 / 2.6.x ladder; Consumers table gained a pending `agnosai` (ZIP read+write, 2.6.0) row. **2.5.4 cut 2026-07-18**: version → 2.5.4, source 11,509 (xz 1,819 / bzip2 1,316), dist full 11,552 / xz 2,778 / bzip2 2,091 / tar 9,466, In-flight slots → 2.5.4 shipped / 2.5.5 next (+ benchmark-doc link). **2.5.6 cut 2026-07-18**: version → 2.5.6, source 12,583 (zstd.cyr 1,798), test 264 fns / 4,483,985, fuzz 5 files / 5,279 iters (+`fuzz_zstd.fcyr`), dist full 12,565 / zstd 1,840 / tar 10,488, In-flight slots → 2.5.6 shipped / 2.5.7 next, SIZE-gate note adds zstd to the informational-exclusion list. **2.5.7 cut 2026-07-18**: version → 2.5.7, source 12,779 (zstd.cyr 1,992), test 265 fns / 4,483,989, dist full 12,761 / zstd 2,034 / tar 10,682, In-flight → 2.5.7 shipped / 2.5.8 optional; Recent-releases + Version-history rows gained 2.5.7. **2.5.8 cut 2026-07-19**: version → 2.5.8, source 12,845 (zstd.cyr 2,058), test 267 fns / 4,484,022 (+ a counting-correction note: historical totals were 21 low), dist full 12,827 / zstd 2,100 / tar 10,748, In-flight rewritten (2.5.8 shipped; the optimal-parse DP deferred with its measured justification), zstd_compress suite description gained the two 2.5.8 regression tests, SIZE-gate note explains the new `zstd6_rec_256K` canary and why the `zstd6_text_*` lines were blind to parse quality. |
...
One filed note so far (`001`, the 2.7.5 chain cutoff). Convention: promote an invariant from inline comment / CLAUDE.md to a numbered architecture note when it burns more than ~30 minutes of debugging time for a contributor.
...
plus the 2.8.0/2.8.1 additions (the hand-assembled `PCLMULQDQ` fold, the GPU block encoder) are
audited together there. It ships as its own release, as 2.6.4 did. Last audit: **2.6.4** (ZIP
