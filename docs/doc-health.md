...
> **Last refresh**: 2026-07-18 (**2.5.3 cut — xz / bzip2 ratio cap**; VERSION / CHANGELOG / state / roadmap all bumped to 2.5.3, source + dist figures re-counted [xz 1,771 / bzip2 1,272; full bundle 11,460], test totals 4,483,866 / fuzz 4,129, INFO-F closed, CLAUDE.md Zstandard hard rule removed + modular-by-profile principle added. Same-day earlier passes: 2.5.2 pin refresh, structural re-count, forward-ladder scheduling.) | **Refresh cadence**: opportunistic — update the affected row whenever a doc is touched.
>
...
| `CHANGELOG.md` | 2026-07-18 | ✅ Fresh | **Source of truth per CLAUDE.md.** Through `[2.5.3] — 2026-07-18` (xz / bzip2 ratio cap `*_decompress_with_ratio_cap`; closes INFO-F); `[2.5.2]` toolchain pin → 6.4.66; `[2.5.1]` per-codec distlib profiles; `[2.5.0]` sovereign zstd + tar cursor. Empty `[Unreleased]` header at top per Keep-a-Changelog convention. |
...
| `VERSION` | 2026-07-18 | ✅ Fresh | `2.5.3`. Single source of truth per the standards. |
