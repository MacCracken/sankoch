...
> **Last refresh**: 2026-07-18 (**2.5.6 cut — zstd encoder competitiveness + decoder hardening**; VERSION / CHANGELOG / state / roadmap bumped to 2.5.6, source re-counted [zstd.cyr 1,462→1,798; total 12,583; full bundle 12,565], test totals 264 fns / 4,483,985, fuzz 5 files / 5,279 iters [+`fuzz_zstd.fcyr`], new benchmark doc `2026-07-18-2.5.6-zstd-competitiveness.md` + security audit `2026-07-18-zstd-decoder-hardening.md`, roadmap 2.5.6 shipped + 2.5.7 repcode-aware matching) | **Refresh cadence**: opportunistic — update the affected row whenever a doc is touched.
>
...
| `CHANGELOG.md` | 2026-07-18 | ✅ Fresh | **Source of truth per CLAUDE.md.** Through `[2.5.6] — 2026-07-18` (zstd encoder competitiveness — FSE literal weights + repeat offsets + lazy parse + level knob; decoder hardening — 36 OOB/DoS paths); `[2.5.5]` sovereign zstd encoder; `[2.5.4]` xz/bzip2 encoder throughput; `[2.5.3]` xz/bzip2 ratio cap; `[2.5.2]` pin → 6.4.66; `[2.5.1]` per-codec distlib profiles; `[2.5.0]` sovereign zstd + tar cursor. Empty `[Unreleased]` header at top per Keep-a-Changelog convention. |
...
| `VERSION` | 2026-07-18 | ✅ Fresh | `2.5.6`. Single source of truth per the standards. |
| `cyrius.cyml` | 2026-07-18 | ✅ Fresh | Toolchain pin `cyrius = "6.4.67"` (bumped from 6.4.66 during the 2.5.6 arc; 2.5.5 shipped on 6.4.66. `cyrius deps` re-resolved, all gates clean). |
