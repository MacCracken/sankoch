...
- **Tag**: `2.7.3` (bare semver, no `v` prefix)
- **Released**: 2026-07-20
...
- **7,499 iterations** across 36 harness functions in 6 files:
  ...
  - `fuzz/fuzz_zstd.fcyr`: 1,650 (2.5.6 — decode-survival on random input 400 + encode→decode round-trip across 5 distributions 600 + corruption of valid streams 150; found the decoder-hardening SIGSEGVs; **2.7.3 — +500 DP-optimal-parse (levels 7–9) round-trips**, each distribution × level)
...
**2.7.3 shipped — the committed 2.7.x performance & ratio ladder is complete.** Four releases
closed the two largest encoder gaps in the tree. The **xz** encoder (2.7.0–2.7.2): repetitive-data
speed (rep-only `nice_len` shortcut, ~290–473×), real-source speed (BT4 binary-tree finder, +21 %;
an HC4 attempt was rejected — the "78 % match finder" was operation-count, not time), real-source
ratio (xz-private window 32 KB → 256 KB) — now within ~0.2 % of `xz -6` on inputs that fit the
window. The **zstd** encoder (2.7.3): a DP optimal parser at levels 7–9 with per-block best-of
(never worse than the greedy default; real-source L9 −4.9 %), validated against reference
...
| Tag    | Date       | Headline                                              |
|--------|------------|-------------------------------------------------------|
| 2.7.3  | 2026-07-21 | **zstd encoder DP optimal parse (levels 7–9)** — shortest-path parse with per-node repcode state, gated behind `zstd_compress_level` 7–9; **per-block best-of** keeps it a strict improvement over the greedy default (real-source L9 −4.9 %, records best-of-neutral). Validated vs reference `zstd -d` (40 adversarial files + 800 round-trips); +500 fuzz round-trips + L9 smoke. **Completes the 2.7.x ladder.** Surfaced the zstd 128 KB window as the record-data limit (→ conditional) |
