...
- **Tag**: `2.6.2` (bare semver, no `v` prefix)
- **Released**: 2026-07-19
...
|-----------------------------------------------|----------:|-----------:|
| `tests/tcyr/*.tcyr` (21 split suites)         |       273 |  4,137,827 |
...
| Bundle                       | Lines | Role |
|------------------------------|------:|------|
| `dist/sankoch.cyr`           | 14,360 | Full library — LZ4 / LZ4F / DEFLATE / zlib / gzip / xz / bzip2 de/compress + zstd de/compress (encode 2.5.5, competitive 2.5.6–2.5.8) + tar cursor, batch + streaming, + ratio-capped decompress (DEFLATE family batch + streaming; xz + bzip2 batch, 2.5.3) |
...
| `dist/sankoch-zip.cyr`       | 5,178 | **[lib.zip]** (2.6.0) — PKZIP `.zip` container: in-memory reader + writer, methods 0 (store) / 8 (DEFLATE), CRC-verified, zip-slip guards, per-member ratio cap. The DEFLATE closure + crc32 + `zip.cyr`; excludes `tar.cyr`. agnosai's `.agpkg` profile |
| `dist/sankoch-zipall.cyr`    | 10,876 | **[lib.zipall]** (2.6.1) — ZIP with EVERY method sankoch owns: 0 / 8 / 12 (bzip2) / 93 (zstd) / 95 (xz), read + write. Adds `zip_methods.cyr` + the xz/bzip2/zstd codecs to the `[lib.zip]` closure. Use `[lib.zip]` when only store + DEFLATE are needed — it is less than half the size |
...
**2.6.2 shipped (2026-07-19) — ZIP carries every method sankoch owns, plus Zip64.** The pre-2.6.0 P(-1) pass (2.5.9 +
...
extended-information extra field — lifting the 65,535-member and 4 GB ceilings. Reference
parity verified both directions (unzip + bsdtar + Python `zipfile`) across ten checks.
...
- **2.6.3 — streaming + metadata.** Streaming read + streaming write (data descriptors,
  ...
  metadata (mtime, mode, symlink) for tar-parity extraction. The writer still emits a
  fixed 1980-01-01 MS-DOS date and no external attributes — this is the bite that changes
  that. **The last scheduled bite of the ZIP arc.**
