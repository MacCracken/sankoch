...
- **Tag**: `2.6.4` (bare semver, no `v` prefix)
- **Released**: 2026-07-20
...
**2.6.4 shipped — the ZIP surface is P(-1)-audited and cleared.** The 2.6.x arc built a
complete PKZIP container (2.6.0 store+DEFLATE reader+writer → 2.6.1 methods 12/93/95 →
2.6.2 Zip64 → 2.6.3 streaming + Unix metadata), all reference-verified both ways against
unzip / bsdtar / Python `zipfile`. That added ~1,900 never-audited lines of `zip.cyr` +
`zip_methods.cyr`; 2.6.4 ran the P(-1) security pass over them (mirroring the pre-2.6.0
codec audit) and closed every confirmed finding — **0 HIGH · 3 MEDIUM · 1 LOW** (dossier
[`docs/audit/2026-07-20-zip-container.md`](../audit/2026-07-20-zip-container.md)). The
one HIGH claim (cross-entry symlink escape by analogy to the tar H-1) was **rebased to
LOW**: the ZIP surface is purely in-memory and performs zero filesystem operations, so the
tar-style on-disk traversal has no reachable analogue; the archive-global symlink ledger
(FNV-1a hash set) is kept as defense-in-depth. All fixes landed and regression-tested;
`fuzz/fuzz_zip.fcyr` merged (raising the fuzz harness count 5 → 6).

**Forward: the committed 2.7.x performance & ratio ladder** (forward detail in
[`roadmap.md`](roadmap.md#-scheduled--the-27x-performance--ratio-ladder)) — the
formerly-scattered deferred items, promoted highest-value first:

- **2.7.0 — xz encoder match finder.** The 2.5.9 baseline measured xz encode at
  ~0.07–0.16 MB/s, **~400–900× slower than `xz -6`** for equal-or-better ratio; reference
  xz uses a BT4 match finder where sankoch walks a plain chain and prices every position.
  Largest measured gap in the tree; `takumi` is an xz-encode consumer. A new match finder,
  not micro-optimisation. Baseline:
  [`docs/benchmarks/2026-07-19-2.5.9-p1-baseline.md`](../benchmarks/2026-07-19-2.5.9-p1-baseline.md).
- **2.7.1 — zstd optimal / 2-pass parse.** The residue on very-regular record data. A
  working DP probe (preserved at `/home/macro/Repos/sankoch-deferred-dp-2.5.8.diff`)
  reached −3.6 % on real source/binary vs the 2.5.8 priced parse's −0.5 %, at ~400 lines /
  ~224 KiB DP arrays / 4–74× encode time — worth doing gated behind levels 7–9.
