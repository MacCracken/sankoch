...
- **Tag**: `2.6.3` (bare semver, no `v` prefix)
- **Released**: 2026-07-19
...
|-----------------------------------------------|----------:|-----------:|
| `tests/tcyr/*.tcyr` (21 split suites)         |       277 |  4,137,879 |
...
| Bundle                       | Lines | Role |
|------------------------------|------:|------|
| `dist/sankoch.cyr`           | 14,648 | Full library — LZ4 / LZ4F / DEFLATE / zlib / gzip / xz / bzip2 de/compress + zstd de/compress (encode 2.5.5, competitive 2.5.6–2.5.8) + tar cursor, batch + streaming, + ratio-capped decompress (DEFLATE family batch + streaming; xz + bzip2 batch, 2.5.3) |
...
| `dist/sankoch-zip.cyr`       | 5,461 | **[lib.zip]** (2.6.0) — PKZIP `.zip` container: in-memory reader + writer, methods 0 (store) / 8 (DEFLATE), CRC-verified, zip-slip guards, per-member ratio cap. The DEFLATE closure + crc32 + `zip.cyr`; excludes `tar.cyr`. agnosai's `.agpkg` profile |
| `dist/sankoch-zipall.cyr`    | 11,164 | **[lib.zipall]** (2.6.1) — ZIP with EVERY method sankoch owns: 0 / 8 / 12 (bzip2) / 93 (zstd) / 95 (xz), read + write. Adds `zip_methods.cyr` + the xz/bzip2/zstd codecs to the `[lib.zip]` closure. Use `[lib.zip]` when only store + DEFLATE are needed — it is less than half the size |
...
**2.6.3 shipped (2026-07-19) — the 2.6.x ZIP arc is COMPLETE.** The pre-2.6.0 P(-1) pass (2.5.9 +
...
and a per-member ratio cap — **the whole agnosai `.agpkg` filing**. 2.6.1 added methods 12 / 93 / 95 on both sides via `zip_methods.cyr`, which is excluded
from the lean `[lib.zip]` profile so agnosai never pays for codecs it does not use.
2.6.2 added Zip64 on both sides — the EOCD record + locator and the
extended-information extra field — lifting the 65,535-member and 4 GB ceilings. 2.6.3
closed the arc with a streaming writer (bit-3 + data descriptors, `zip_enc_*`) and
per-entry Unix metadata (mode / mtime / symlink), so a consumer gets tar-parity
extraction. Reference parity verified both directions (unzip + bsdtar + Python `zipfile`)
across thirteen checks; bsdtar restores real symlinks and modes from sankoch's output.

**No ZIP work is scheduled.** Encryption, multi-disk/spanned archives and Deflate64 remain
non-goals. What is left on the ladder is the deferred bucket below plus the next P(-1)
hardening pass, which should audit the ~1,900 lines of `zip.cyr` + `zip_methods.cyr` the
2.6.x arc added — that surface has not had a security audit.
