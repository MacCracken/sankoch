...
- **Tag**: `2.7.2` (bare semver, no `v` prefix)
- **Released**: 2026-07-20
...
**2.7.2 shipped — the xz-encode arc is complete.** Across 2.7.0–2.7.2 the .xz encoder — the
largest measured perf gap in the tree — was closed on three axes: **2.7.0** repetitive-data speed
(rep-only `nice_len` greedy shortcut, ~290–473× faster), **2.7.1** real-source speed (BT4
binary-tree finder, +21 %; an HC4 attempt was rejected — the "78 % match finder" figure was
operation-count, not time), **2.7.2** real-source ratio (xz-private match window 32 KB → 256 KB
+ matching 256 KB LZMA2 dict). Real-source corpus is now **within ~0.2 % of `xz -6`** on ratio (was
+7 %) at ~5.8× its encode speed. DEFLATE's shared 32 KB lz77 stayed frozen throughout (the finder
+ window are xz-private). Every stream round-trips through reference `xz -d`; the son[] grew to
8 MB (lazy, encode-only). Each release was research-backed and adversarially reviewed via a
workflow before code landed.
