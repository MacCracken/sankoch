...
**2.7.0 shipped — the xz-encode repetitive-data speedup.** The .xz encoder was the largest
measured perf gap in the tree. Profiling
([`docs/benchmarks/2026-07-20-2.7.0-baseline.md`](../benchmarks/2026-07-20-2.7.0-baseline.md))
split it into two disjoint regimes — **repetitive** data (300–750× slower than `xz -6`; cost =
the windowed optimal parse pricing a ~273-length match at every one of the 4096 window
positions) and **real source** (7.3× slower; cost = the HC3 hash-chain walk, ~78 %). 2.7.0
ships the repetitive fix (the bigger prize, and the one the roadmap had *not* named — it assumed
"match finder"): a **rep-only `nice_len` greedy shortcut** + an **interior DP cut**, both gated
on a *saturated* rep — the ratio-safe subset an adversarial design review isolated (a
normal-match greedy regresses real-source ratio). Result: text/zeros **~290–473× faster and
smaller**, corpus exactly neutral; every stream still round-trips via reference `xz -d`. The
research + adversarial design ran as a workflow before any code landed.

**Forward: the 2.7.x ladder** (forward detail in
[`roadmap.md`](roadmap.md#-scheduled--the-27x-performance--ratio-ladder)):

- **2.7.1 — xz encoder HC4 match finder.** The real-source regime 2.7.0 left untouched: corpus
  0.59 vs `xz -6` 4.4 MB/s (7.3×), ~78 % HC3 chain walk. Fix = a 4-byte main hash + 3-byte seed
  + `nice_len` early-out on **xz-private** tables (DEFLATE's shared 3-byte lz77 untouched). A
  full adversarially-reviewed design (byte sizes, mask-before-multiply hash, `best >= maxlen`
  over-read guard, the load-bearing `dist > WINDOW` dict reject) is in the 2.7.0 design dossier.
- **2.7.2 — zstd optimal / 2-pass parse.** The residue on very-regular record data. A working DP
